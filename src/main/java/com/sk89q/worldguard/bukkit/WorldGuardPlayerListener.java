...
public class WorldGuardPlayerListener extends PlayerListener {
	...
	private void registerEvent(String typeName, Priority priority) {
		try {
			...
			logger.info("WorldGuard: Unable to register missing event type "
					+ typeName);
		}
	}
	...
	public void onPlayerJoin(PlayerJoinEvent event) {
		...
		if (cfg.activityHaltToggle) {
			...
			if (removed > 10) {
				logger.info("WG Halt-Act: " + removed
						+ " entities (>10) auto-removed from "
		...
		if (plugin.inGroup(player, "wg-invincible")
				|| (cfg.autoGodMode && plugin.hasPermission(player,
		...
		if (wcfg.useRegions) {
			PlayerFlagState state = plugin.getFlagStateManager().getState(
					player);
	...
	public void onPlayerQuit(PlayerQuitEvent event) {
		...
		if (wcfg.useRegions) {
			boolean hasBypass = plugin.getGlobalRegionManager().hasBypass(
					player, world);
			PlayerFlagState state = plugin.getFlagStateManager().getState(
					player);
			...
			if (state.lastWorld != null && !hasBypass) {
				...
				Location loc = player.getLocation();
				Vector pt = new Vector(loc.getBlockX(), loc.getBlockY(),
						loc.getBlockZ());
				ApplicableRegionSet set = mgr.getApplicableRegions(pt);

				if (state.lastExitAllowed == null) {
					state.lastExitAllowed = set.allows(DefaultFlag.EXIT,
							localPlayer);
				}

				if (!state.lastExitAllowed
					...
					if (state.lastWorld.equals(world)) {
						Location newLoc = new Location(world,
								state.lastBlockX + 0.5, state.lastBlockY,
								state.lastBlockZ + 0.5);
	...
	public void onPlayerMove(PlayerMoveEvent event) {
		...
		if (wcfg.useRegions) {
			...
			if (event.getFrom().getBlockX() != event.getTo().getBlockX()
				...
				boolean entryAllowed = set.allows(DefaultFlag.ENTRY,
						localPlayer);
				...
				if (state.lastExitAllowed == null) {
					state.lastExitAllowed = mgr.getApplicableRegions(
							toVector(event.getFrom())).allows(DefaultFlag.EXIT,
							localPlayer);
				}
				...
				if (!hasBypass && exitAllowed && !state.lastExitAllowed) {
					player.sendMessage(ChatColor.DARK_RED
							+ "You are not permitted to leave this area.");
				...
				for (ProtectedRegion region : set) {
					if (region.getFlag(DefaultFlag.TELE_LOC) == null
							|| region.getFlag(DefaultFlag.TELE_PERM) == null) {
						continue;
					}
					boolean shouldTeleport = false;
					switch (region.getFlag(DefaultFlag.TELE_PERM)) {
					case MEMBERS:
						shouldTeleport = region.isMember(localPlayer);
						break;
					case NON_MEMBERS:
						shouldTeleport = !region.isMember(localPlayer);
						break;
					case OWNERS:
						shouldTeleport = region.isOwner(localPlayer);
						break;
					case NON_OWNERS:
						shouldTeleport = !region.isOwner(localPlayer);
						break;
					...
					if (shouldTeleport) {
						Vector loc = region.getFlag(DefaultFlag.TELE_LOC);
				...
				if (state.lastFarewell != null
						&& (farewell == null || !state.lastFarewell
								.equals(farewell))) {
					String replacedFarewell = plugin.replaceMacros(player,
							BukkitUtil.replaceColorMacros(state.lastFarewell));
					player.sendMessage(ChatColor.AQUA + " ** "
							+ replacedFarewell);
				}

				if (greeting != null
						&& (state.lastGreeting == null || !state.lastGreeting
								.equals(greeting))) {
					String replacedGreeting = plugin.replaceMacros(player,
							BukkitUtil.replaceColorMacros(greeting));
					player.sendMessage(ChatColor.AQUA + " ** "
							+ replacedGreeting);
				}

				if ((notifyLeave == null || !notifyLeave)
						&& state.notifiedForLeave != null
						&& state.notifiedForLeave) {
				...
				if (notifyEnter != null
						&& notifyEnter
						&& (state.notifiedForEnter == null || !state.notifiedForEnter)) {
	...
	private void handleBlockLeftClick(PlayerInteractEvent event) {
		...
		if (wcfg.useRegions) {
			...
			if (type == Material.STONE_BUTTON || type == Material.LEVER
				...
				if (!plugin.getGlobalRegionManager().hasBypass(player, world)
					...
					player.sendMessage(ChatColor.DARK_RED
							+ "You don't have permission to use that in this area.");
		...
		if (wcfg.getBlacklist() != null) {
			if (!wcfg.getBlacklist().check(
					new ItemUseBlacklistEvent(plugin.wrapPlayer(player),
							toVector(player.getLocation()), item.getTypeId()),
					false, false)) {
				event.setCancelled(true);
	...
	private void handleBlockRightClick(PlayerInteractEvent event) {
		...
		if ((type == Material.CHEST || type == Material.JUKEBOX
				|| type == Material.DISPENSER || type == Material.FURNACE || type == Material.BURNING_FURNACE)
			...
			for (int slot = 0; slot < 40; slot++) {
				...
				if (heldItem != null && heldItem.getAmount() < 0) {
					...
					player.sendMessage(ChatColor.RED
							+ "Infinite stack in slot #" + slot + " removed.");
				}
			}
		}
		...
		if (wcfg.useRegions) {
			...
			if (item.getTypeId() == wcfg.regionWand) {
				if (set.size() > 0) {
					...
					for (Iterator<ProtectedRegion> it = set.iterator(); it
							.hasNext();) {
					...
					player.sendMessage(ChatColor.YELLOW
							+ "Applicable regions: " + str.toString());
				} else {
					player.sendMessage(ChatColor.YELLOW
			...
			if (item.getType() == Material.INK_SACK && item.getData() != null
			...
			if (type == Material.CHEST
					...
					|| type == Material.DISPENSER || type == Material.FURNACE
					|| type == Material.BURNING_FURNACE) {
				if (!plugin.getGlobalRegionManager().hasBypass(player, world)
						&& !set.allows(DefaultFlag.CHEST_ACCESS)
						&& !set.canBuild(localPlayer)) {
					player.sendMessage(ChatColor.DARK_RED
							+ "You don't have permission to open that in this area.");
					event.setCancelled(true);
					return;
				}
			}
			...
			if (type == Material.LEVER || type == Material.STONE_BUTTON
					...
					|| type == Material.WOODEN_DOOR
					|| type == Material.TRAP_DOOR || type == Material.WORKBENCH) {
				if (!plugin.getGlobalRegionManager().hasBypass(player, world)
						&& !set.allows(DefaultFlag.USE)
						&& !set.canBuild(localPlayer)) {
					player.sendMessage(ChatColor.DARK_RED
			...
			if (type == Material.CAKE_BLOCK) {
				if (!plugin.getGlobalRegionManager().hasBypass(player, world)
						&& !set.canBuild(localPlayer)) {
					player.sendMessage(ChatColor.DARK_RED
							+ "You're not invited to this tea party!");
					event.setCancelled(true);
					return;
				}
			}
			...
			if ((type == Material.RAILS || type == Material.POWERED_RAIL || type == Material.DETECTOR_RAIL)
				...
				if (!plugin.getGlobalRegionManager().hasBypass(player, world)
					...
					player.sendMessage(ChatColor.DARK_RED
							+ "You don't have permission to place vehicles here.");
					event.setCancelled(true);
					return;
				}
			}
		...
		if (wcfg.getBlacklist() != null) {
			if ((block.getType() != Material.CHEST
				...
				if (!wcfg.getBlacklist().check(
						new ItemUseBlacklistEvent(plugin.wrapPlayer(player),
								toVector(block), item.getTypeId()), false,
						false)) {
					event.setCancelled(true);
					return;
				}
				...
				if (!wcfg.getBlacklist().check(
						new BlockInteractBlacklistEvent(
								plugin.wrapPlayer(player), toVector(block),
		...
		if (wcfg.getBlacklist() != null) {
			...
			if (!wcfg.getBlacklist().check(
					new ItemAcquireBlacklistEvent(plugin.wrapPlayer(event
							.getPlayer()), toVector(ci.getLocation()), ci
							.getItemStack().getTypeId()), false, true)) {
				event.setCancelled(true);
				return;
			}
		}
	...
	public void onPlayerBucketFill(PlayerBucketFillEvent event) {
		...
		if (!plugin.getGlobalRegionManager().canBuild(player,
		...
		if (wcfg.getBlacklist() != null) {
			if (!wcfg.getBlacklist().check(
					new ItemUseBlacklistEvent(plugin.wrapPlayer(player),
							toVector(player.getLocation()), event.getBucket()
									.getId()), false, false)) {
				event.setCancelled(true);
				return;
			}
	...
	public void onPlayerRespawn(PlayerRespawnEvent event) {
		...
		if (wcfg.useRegions) {
			...
			if (spawn != null) {
				...
				Location spawnLoc = BukkitUtil.toLocation(player.getWorld(),
						spawn);
	...
	public void onPlayerBedEnter(PlayerBedEnterEvent event) {
		...
		if (wcfg.useRegions) {
			...
			if (!plugin.getGlobalRegionManager().hasBypass(player,
					player.getWorld())
	...
	public void onPlayerCommandPreprocess(PlayerCommandPreprocessEvent event) {
		...
		if (wcfg.useRegions
				&& !plugin.getGlobalRegionManager().hasBypass(player, world)) {
			...
			if (allowedCommands != null
					&& !allowedCommands.contains(parts[0].toLowerCase())) {
				player.sendMessage(ChatColor.RED + parts[0].toLowerCase()
			...
			if (blockedCommands != null
					&& blockedCommands.contains(parts[0].toLowerCase())) {
				player.sendMessage(ChatColor.RED + parts[0].toLowerCase()
